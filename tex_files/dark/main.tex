...
\begin{exercise} \\
    ...
    $$\varphi_x(y_1, y_2, y_3, ... y_k) = \varphi_{h(y_1, y_2, ... y_k)}(x)$$
    Define a new function $H$ as follows -
    $$H(y) = f(y, x, x, x, ..., x)$$
    And a function $g$ as follows - 
    $$g(x) = \begin{dcases}
        1 & \text{if } H(h(x, ..., x)) = 1 \\
        0 & \text{otherwise}
        \end{dcases}$$
    We can see that $g$ is a recursive function, since $H$ is a recursive function. But by the definition of $h$, we have - 
    $$g(x) = 1 \iff f(h(x, ..., x), x, ..., x) = 1 \iff \varphi_{h(x, ..., x)}(x)\downarrow \iff \varphi_{x}(x, ..., x) \downarrow$$ 
    This leads to a contradiction, since it implies that $g$ can be used to determine whether $\varphi_x(x, ..., x)$ converges or not, which is the diagonal Halting Problem. Hence, no such function $f$ exists.
\end{exercise}
\begin{exercise} \\
